<template>
  ...
  <template v-if="props.col.actions.length > 10">
    <q-btn-dropdown flat no-caps no-wrap  :color="menuColor">
      <template v-slot:tooltip>
        <q-tooltip>More actions</q-tooltip>
      </template>
      <q-list>
        <q-item clickable v-close-popup v-for="(action, index) in props.col.actions" :key="index" @click="action.action(props)">
          <q-item-section>
            <q-icon :name="action.icon ? action.icon : handleIcon(props.col.options.find((o) => o.value == props.row[props.col.field])?.value)"></q-icon>
            <q-item-label>{{ action.toolTip }}</q-item-label>
          </q-item-section>
        </q-item>
      </q-list>
    </q-btn-dropdown>
  ...
  <template v-else>
    <template v-for="(action, index) in props.col.actions" :key="index">
      <q-btn flat round padding="xs" size="sm" :color="action.color" :icon="action.icon ? action.icon : handleIcon(props.col.options.find((o) => o.value == props.row[props.col.field])?.value)" @click="action.action(props)">
        ...
        <template v-else-if="props.col.type == 'name'">
...
const formatDate = (dateInput) => {
  ...
  // Ajusta o mês (getUTCMonth() retorna de 0 a 11)
